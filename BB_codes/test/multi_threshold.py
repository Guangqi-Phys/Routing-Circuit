...
if __name__ == "__main__":
    ...
    sinter.Task(
        circuit = si1000_noise_model(
            gen_circ_only_z_detectors(BBCode(get_config(code_setting).get_params()), BBCode(get_config(code_setting).get_params()).qcodedz), BBCode(get_config(code_setting).get_params()).full_qubit_set,
            probability=noise,
        ),
        json_metadata={'code': code_setting, 'r': BBCode(get_config(code_setting).get_params()).qcodedz, 'p': noise},
    )
    for code_setting in [5]
    for noise in [0.0005, 0.001, 0.003, 0.004]  
    ]
    ...
    with open(os.path.join(results_dir, 'collected_bb_stats_reduced_coupler5.pkl'), 'wb') as f:
